...
parser.add_argument('genMatedScoresFile', help='filename for the Gen-mated scores', type=str)
...
genMatedScoresFile = args.genMatedScoresFile
...
bin_edges = numpy.linspace(min([min(matedScores), min(nonMatedScores), min(genMatedScores)]), max([max(matedScores), max(nonMatedScores), max(genMatedScores)]), num=nBins + 1, endpoint=True)
...
y3 = numpy.histogram(genMatedScores, bins = bin_edges, density = True)[0]
