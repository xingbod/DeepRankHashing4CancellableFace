    ...
    if dataset == 'VGG2':
        for i in range(totalsamples):
            if tf.__version__.startswith('1'):  # important if you want to run with tf1.x,
                labels.append(tf.compat.v1.string_to_number(tf.compat.v1.strings.substr(tf.string_split(imgs[None, i], os.path.sep).values[-2],pos=1, len=6),
        ...
        for i in range(totalsamples):
            ...
            if tf.__version__.startswith('1'):  # important if you want to run with tf1.x,
                labels.append(tf.compat.v1.string_to_number(tf.string_split(imgs[None, i], os.path.sep).values[-2],
                                                            out_type=tf.dtypes.int32))
            else:
                labels.append(
...
def load_online_pair_wise_dataset(dbdir,ext = 'jpg',dataset_ext = 'ms',samples_per_class = 3,classes_per_batch = 4,is_ccrop = False, buffer_size=50240):
    ...
    ds = path_ds.shuffle(buffer_size).interleave(lambda x: ListFiles(x, ext), cycle_length=3000,# every time, cycle_length classes are taken out to do the maping MS 85742, here at most 3k, otherwise machine crash
