    ...
    if dataset == 'VGG2':
        ...
        for i in range(totalsamples):
            ...
            if tf.__version__.startswith('1'):  # important is you want to run with tf1.x,
                print('********', tf.compat.v1.strings.split(imgs[None, i], os.path.sep))
                labels.append(tf.compat.v1.string_to_number(tf.string_split(imgs[None, i], os.path.sep).values[0, -2],
