...
for cnt_i=1:1
    ...
    [identifiers ] = generate_identifier2(512,8,1);
    Descriptors2 = bitxor(Descriptors,identifiers);
    dist_imposter = 1-pdist2(Descriptors,Descriptors2,'Hamming');
...
%
    ...
    imposter_mated = dist_imposter(mask);
    imposter = dist_gen(mask==0);
    imposter = imposter(randperm(174614542,length(genuine)));

    dlmwrite('permlut_genuine_id.txt',genuine,'-append')
    dlmwrite('permlut_imposter_mated_id.txt',imposter_mated,'-append')
    dlmwrite('permlut_imposter_id.txt',imposter,'-append')
