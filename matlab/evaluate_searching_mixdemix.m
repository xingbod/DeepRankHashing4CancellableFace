function [perf] = evaluate_searching_mixdemix(hash_facenet_probe_c,hash_facenet_probe_o1,hash_facenet_probe_o2,hash_facenet_probe_o3,hash_facenet_gallery,facenet_probe_label_c,facenet_probe_label_o1,facenet_probe_label_o2,facenet_probe_label_o3, facenet_gallery_label)
...
%% generate identifier, dimension same to hash code
[identifiers ] = generate_identifier2(m,q,12000);
%%% mixing gallery
...
for i = progress(1:size(facenet_probe_label_c,2))
    ...
    for j=1: size(mixing_facenet_gallery,1)
        ...
        retrieved_id = xor(gallery_bin,hash_facenet_probe_c(i,:));
        dist(j) = sum(xor(retrieved_id,identifiers(facenet_gallery_label(j),:)))/m;
        %dist(j) = pdist2( retrieved_id,identifiers(facenet_gallery_label(j),:),  measure);% it may be slow?
    end
    final_dist(i,:) = dist;
