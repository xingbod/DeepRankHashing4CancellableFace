...
for m = [64 128 256 512]
    for q = [8]
        tic
        enroll_query_iom_id(embpath+"/"+backbone+"_"+embed+"_feat_dlIoM_"+num2str(m)+"x"+num2str(q)+".csv",  'embeddings_dl/'+backbone+'_'+embed+'_name'+correct_str+'_'+num2str(m)+"x"+num2str(q)+'.txt',measure,DS,remark);
        toc
    end
end
